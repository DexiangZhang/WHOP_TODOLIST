...
import ItemCSS from "../CSS/todoListItem.module.css";

const TodoListItem = ({ todo, onRemovePressed, uniqueID }) => {
  const { text, id } = todo;
  ...
  return (
    <div>
      <div className={ItemCSS.item}>
        <span key={uniqueID} className={ItemCSS.title}>
          {text}
        </span>
      ...
      {click ? (
        <div className={ItemCSS.container}>
          <div className={ItemCSS.space}> </div>
          <div className={ItemCSS.options}>
            <Link
              to="/editToDo"
              state={{ text: text, id: id }}
              className={ItemCSS.link}
            >
              <svg
                width="18"
                height="18"
